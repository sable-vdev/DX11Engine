...
Mat4 Camera::s_viewMatrix = {};
Mat4 Camera::s_projectionMatrix = {};
Mat4 Camera::s_orthographicMatrix = {};

CBDCamera Camera::s_cameraConstantInfo = CBDCamera{};

Camera::Camera(I32 width, I32 height) : m_position(0.0f, 0.0f, -20.0f, 0.0f), m_rotation(0.0f, 0.0f, 0.0f)
{
	m_aspect = static_cast<float>(width) / static_cast<float>(height);

	DX::XMStoreFloat4x4(&s_projectionMatrix, DX::XMMatrixPerspectiveFovLH(m_FOV, m_aspect, m_nearZ, m_farZ));
	DX::XMStoreFloat4x4(&s_orthographicMatrix, DX::XMMatrixOrthographicOffCenterLH(0, static_cast<float>(width), static_cast<float>(height), 0, m_nearZ, m_farZ));
...
void Camera::Update(float dt)
{
	...
	DX::XMStoreFloat4x4(&s_viewMatrix, DX::XMMatrixLookAtLH(pos, target, s_up));
